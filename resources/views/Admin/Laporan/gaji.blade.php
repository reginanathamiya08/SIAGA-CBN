...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
        <form method="GET" action="{{ route('admin.laporan.gaji.index') }}"
            ...
            @if($slipGaji->count() > 0)
            <div>
                <a href="{{ route('admin.laporan.gaji.export', ['periode_id' => $periodeId, 'mitra_id' => $mitraId]) }}"
                   target="_blank"
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-blue-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $slip->karyawan?->user?->username ?? '-' }}</p>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right font-mono text-slate-600">
                            {{ number_format($slip->uang_makan + $slip->uang_transport, 0, ',', '.') }}
                        </td>
