        ...
        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-50 flex justify-between items-center">
                ...
                <span class="text-[9px] font-black text-gray-400">
                    {{ $mitra->penempatan->count() + $karyawanTetap->count() }} orang
                </span>
            </div>
            ...
            {{-- Loop Karyawan Tetap (Khusus Pusat) --}}
            @foreach ($karyawanTetap as $tetap)
                <div class="flex items-center justify-between px-6 py-3.5
                            border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-all">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 bg-blue-100 text-blue-700 rounded-xl
                                    flex items-center justify-center font-black text-xs shrink-0">
                            {{ strtoupper(substr($tetap->nama, 0, 2)) }}
                        </div>
                        <div>
                            <p class="text-[11px] font-black text-[#1E3A5F]">
                                {{ $tetap->nama }}
                            ...
                            <p class="text-[9px] text-gray-400 font-medium">
                                {{ $tetap->jabatan }} • Karyawan Tetap
                            </p>
                        </div>
                    </div>
                    <div>
                        <span class="px-2 py-1 bg-blue-50 text-blue-600 rounded-lg text-[9px] font-black uppercase tracking-wider">
                            Pusat
                        </span>
                    </div>
                </div>
            ...
            @if($mitra->penempatan->isEmpty() && $karyawanTetap->isEmpty())
