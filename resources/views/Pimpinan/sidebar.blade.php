...
<body class="bg-gray-50 min-h-screen" x-data="{ sidebarOpen: true }">
    ...
    <aside class="fixed top-0 left-0 h-full bg-[#1E3A5F] text-white z-40
        ...
        <div class="flex items-center gap-3 px-4 py-5 border-b border-white/10">
            ...
            <div x-show="sidebarOpen" x-transition.opacity>
                ...
                <p class="text-[9px] text-white/50 font-medium mt-0.5">Pimpinan</p>
            </div>
        </div>
            ...
            <div class="px-3 space-y-0.5">
                ...
                <a href="{{ route('pimpinan.dashboard') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all
                          {{ request()->routeIs('pimpinan.dashboard') ? 'bg-white/15 text-white' : 'text-white/60 hover:bg-white/10 hover:text-white' }}">
                ...
                {{-- Approval dengan badge jumlah menunggu --}}
                <a href="{{ route('pimpinan.approval.index') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all
                          {{ request()->routeIs('pimpinan.approval.*') ? 'bg-white/15 text-white' : 'text-white/60 hover:bg-white/10 hover:text-white' }}">
                    <i data-lucide="check-square" class="w-4 h-4 shrink-0"></i>
                    <span x-show="sidebarOpen" x-transition.opacity class="text-[11px] font-black uppercase tracking-wider flex-1">
                        Approval Pengajuan
                    </span>
                    @php
                        $totalMenunggu = \App\Models\Perizinan::where('status_approval','menunggu')->count()
                                       + \App\Models\Lembur::where('status_approval','menunggu')->count()
                                       + \App\Models\DinasLuar::where('status_approval','menunggu')->count();
                    @endphp
                    @if ($totalMenunggu > 0)
                        <span x-show="sidebarOpen"
                              class="bg-red-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded-full shrink-0">
                            {{ $totalMenunggu }}
                        </span>
                    @endif
                </a>
        ...
        <div class="border-t border-white/10 p-3">
            <div class="flex items-center gap-3 px-3 py-2">
                <div class="w-8 h-8 bg-blue-500 rounded-xl flex items-center justify-center shrink-0 font-black text-xs text-white">
                ...
                <div x-show="sidebarOpen" x-transition.opacity class="flex-1 min-w-0">
                    ...
                    <p class="text-[9px] text-white/40 font-medium">Pimpinan</p>
                </div>
    ...
    <div class="transition-all duration-300" :class="sidebarOpen ? 'ml-64' : 'ml-16'">
        <div class="sticky top-0 z-30 bg-white/80 backdrop-blur-sm border-b border-gray-100 px-6 py-3 flex items-center gap-4">
